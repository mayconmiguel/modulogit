...
<div id="novo<?php echo $tabela;?>">
	<div class="row">
		...
		<div class="col-sm-4 col-xs-12">
			Períodos<input id="periodos" type="text" name="periodos" class="form-control wd100">
		</div>
	</div>
	<div class="row">
		<div class="col-xs-6">
			Rep. Padrão %<input class="form-control" name="rep_padrao" id="rep_padrao">
		</div>
		<div class="col-xs-6">
			Rep. Dif. %<input class="form-control" name="rep_padrao" id="rep_diferenciado">
		</div>
	</div>
	<div class="row">
		<div class="col-xs-12">
			Disciplinas
			<select id="disciplinas" multiple style="width: 100%; height: auto" class="select2">
			</select>
		</div>
	</div>
</div>
...
<script>
	$("#periodos").mask('00');
	$("#rep_padrao,#rep_padrao").mask('000.00',{reverse:true});
	$.post("server/recupera.php",{tabela:'disciplina where grp_emp_id = <?php echo $empresa;?>'}).done(function(data){
		var obj = JSON.parse(data);
		for(c in obj) {
			$("#disciplinas").append('<option value="'+obj[c].id+'">' +
				obj[c].nome +
				'</option>');
		}
		$("#disciplinas,#tipo").select2();
	});
</script>
<?php
...
if(isset($_POST['id'])){
	...
	<script>
		...
		$.post("server/recupera.php",{tabela:'<?php echo $tabela." where grp_emp_id = $empresa and id = ".$id;?>'}).done(function(data){
			...
			//alimenta formulário;
			$("#nome").val(obj[0].nome);
			$("#periodos").val(obj[0].periodos);
			$("#rep_padrao").val(obj[0].rep_padrao);
			$("#rep_diferenciado").val(obj[0].rep_diferenciado);
			$("input[name='tipo[]'][value='"+obj[0].tipo+"']").prop('checked',true).change();
			$("#nome").val(obj[0].nome);
			$("#nome").attr("cod",obj[0].id);
			var disciplinas = obj[0].disciplinas.split(',');
			for(i in disciplinas){
				$("#disciplinas option[value='"+disciplinas[i]+"']").attr("selected",true).change();
			}
		});
		...
		$("#salvar").click(function(){
			...
			var rep_diferenciado = $("#rep_diferenciado").val();
			var rep_padrao	= $("#rep_padrao").val();
			var disciplinas = [];
			$("#disciplinas option:selected").each(function(){
			...
			if(nome==="")
			{
				...
				alerta("AVISO","Favor selecionar pelo menos uma disciplina para continuar","warning","warning");
				$("#disciplinas").focus();
			}
			else if(tipo == undefined || tipo.length == 0){
				alerta("AVISO","Favor escolha o tipo para este curso.","warning","warning");
			}
			else
			{
				loading('show');
				$.ajax({
					...
					type: 'POST',
					cache: false,
					data: {funcao:2,id:"<?php echo $id;?>",nome:nome,rep_padrao:rep_padrao,rep_diferenciado:rep_diferenciado,disciplinas:disciplinas,periodos:periodos,tipo:tipo},
					success: function(data) {
						if(data == 2){
		...
		$("#cadastrar").click(function(){
			...
			else
			{
				...
				$.ajax({
					...
					type: 'POST',
					cache: false,
					data: {funcao:1,tipo:tipo,nome:nome,periodos:periodos,rep_padrao:rep_padrao,rep_diferenciado:rep_diferenciado,disciplinas:disciplinas},
					success: function(data) {
						if(data == 2){
							alerta("AVISO!","Já existe um <?php echo $nome;?> com o mesmo nome cadastrado no sistema!","warning","warning");
							focar($("#nome"));
