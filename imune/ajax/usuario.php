<?php
	...
	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tab." where grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id";
		}else{
			$select = "select ".$colunas." from ".$tab." where (id = '$busca' or nome like '%$busca%') and grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']."";
		}
	}else{
		$select = "select ".$colunas." from ".$tab." where grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id";
	}
												...
												if($explode[$i] == 'status'){
													if($row[$i] == '1'){
														...
														<td style="position: relative;" id="td<?php echo $explode[$i];?>">
															<label class="label bg-color-blue">ATIVO</label>
														</td>
														<?php
													}else{
														?>
														<td style="position: relative;" id="td<?php echo $explode[$i];?>">
															<label class="label bg-color-blueLight">INATIVO</label>
														</td>
														<?php
													}
												}else{
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		$("#todos").click(function(){
			loading('show');
			$.ajax({
				url: 'ajax/<?php echo $tabela;?>.php?busca=PR1SC1L4',
				type: 'GET',
				data:{busca:'PR1SC1L4'},
				cache: false,
		...
		$("#pesquisa").click(function(){
			if($("#busca").val().length < 3){
				...
				$.ajax({
					url: 'ajax/<?php echo $tabela;?>.php?busca='+$("#busca").val(),
					type: 'GET',
					cache: false,
					success: function(data) {
						$("#content").html(data);
