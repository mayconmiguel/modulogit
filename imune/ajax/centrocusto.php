<?php
	...
	$colunas	= "id,nome,naturezas,tipo";
	$tabela		= "centrocusto";

	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tabela." where tipo != 0 and  grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id";
		}else{
			$select = "select ".$colunas." from ".$tabela." where tipo != 0 and (id = '$busca' or nome like '%$busca%') and  grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id";
		}
	}else{
		$select = "select ".$colunas." from ".$tabela." where tipo != 0 and grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id asc limit 50";
	}
								...
								while($row = mysqli_fetch_array($valida)){
									...
									<tr id="<?php echo 'tt'.$row[0];?>" style="cursor:pointer">
										<?php
											$explode = explode(",",$colunas);
											for($i=0;$i<count($explode);$i++){
												if($explode[$i] == 'naturezas'){
													$naturezas = explode(",",$row['naturezas']);
													$nn = "";
													foreach($naturezas as $nat){
														$sel = "select nome from natureza where id = '$nat'";
													...
													$nn = substr($nn,0,strlen($nn)-3);
													$row[$i] = $nn;
												}else if($explode[$i] == 'tipo'){
													if($row[$i] == 1){
														$row[$i] = '<label class="label label-danger">DESPESAS</label>';
													}
													else if($row[$i] == 2){
														$row[$i] = '<label class="label label-success">RECEITAS</label>';
													}
												}
												?>
												<td>
