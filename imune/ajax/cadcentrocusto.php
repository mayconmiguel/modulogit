<?php
$nome = "Centro de Custo"; $tabela = "centrocusto";
...
<div id="novo<?php echo $tabela;?>">
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<select id="naturezas" multiple style="width: 100%; height: auto" class="select2">
			</select>
		</div>
	</div>
</div>
...
<script>
	$.post("server/recupera.php",{tabela:'natureza where grp_emp_id = <?php echo $empresa;?>'}).done(function(data){
...
if(isset($_POST['id'])){
	...
	<script>
		...
		$.post("server/recupera.php",{tabela:'<?php echo $tabela." where grp_emp_id = $empresa and id = ".$id;?>'}).done(function(data){
			...
			//alimenta formulário;
			$("#nome").val(obj[0].nome);
			$("input[name='tipo[]'][value='"+obj[0].tipo+"']").prop('checked',true);
			$("#nome").attr("cod",obj[0].id);
			var naturezas = obj[0].naturezas.split(',');
			for(i in naturezas){
				$("#naturezas option[value='"+naturezas[i]+"']").attr("selected",true).change();
			}
		});
		...
		$("#salvar").click(function(){
			...
			else
			{
				...
				$.ajax({
					...
					type: 'POST',
					cache: false,
					data: {funcao:2,id:"<?php echo $id;?>",nome:nome,tipo:tipo,naturezas:naturezas},
					success: function(data) {
						if(data == 2){
		...
		$("#cadastrar").click(function(){
			...
			else
			{
				...
				$.ajax({
					...
					type: 'POST',
					cache: false,
					data: {funcao:1,nome:nome,tipo:tipo,naturezas:naturezas},
					success: function(data) {
						if(data == 2){
							alerta("AVISO!","Já existe um <?php echo $nome;?> com o mesmo nome cadastrado no sistema!","warning","warning");
							focar($("#nome"));
