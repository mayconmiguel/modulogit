...
$tabelinha  = array("id","data","nome","telefone","celular","Dentista","Convenio","Valor","C.N.C","Status");
?>
...
<!-- widget grid -->
<section id="widget-grid-inscricoes" class="well">
					...
					<div class="widget-body no-padding">
						...
						<table id="datatable_col_reorder" class="table table-striped table-bordered table-hover" width="100%">
							...
							<tr>
								...
								<th class="hasinput">
									<input type="text" class="form-control" placeholder="C.N.C" />
								</th>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/orcamento.php').done(function(data){
		...
		var table = $('#datatable_col_reorder').dataTable({
			...
			"order":[[9,'asc'],[8,'asc'],[1,'desc']],
			"ajax": "server/buscaOrcamento.php",
			"columns": [
				{ "data": "id"},
				{ "data": "dt_cad","sType":"date-uk" },
				{ "data": "nome"},
				{ "data": "celular"},
				{ "data": "email"},
				{ "data": "dentista"},
			...
			"t"+
			"<'dt-toolbar-footer'<'col-sm-4 col-xs-12 hidden-xs'i><'col-sm-4 col-xs-12 hidden-xs'T><'col-sm-4 col-xs-12'p>>",
			"aLengthMenu": [
				[5, 10, 25, 50, -1],
				[5, 10, 25, 50, "TODOS"]
			],
			...
			"oTableTools": {
				"aButtons": [
					{
						"sExtends": "pdf",
						"sTitle": "interessados<?php echo date('YmdHis');?>.PDF",
						"sPdfMessage": "imunevacinas PDF Export",
						"sPdfSize": "A4",
						"sPdfOrientation": "landscape",
					},
					{
						"sExtends": "xls",
						"sTitle": "interessados<?php echo date('YmdHis');?>",
					},
					{
						"sExtends": "csv",
						"sTitle": "interessados<?php echo date('YmdHis');?>",
			...
			"rowCallback" : function(nRow,aData) {
				...
				$(row).attr({
					...
					container: "body",
					animated: true,
					title: "<b>INFORMAÇÕES ADICIONAIS</b>",
					content:"TELEFONE: "+aData.telefone+"<br>CELULAR: "+aData.celular+"<br>EMAIL: "+aData.email+"<br>OBS: "+aData.obs,
					placement: "bottom",
					trigger: "hover",
					html:true
				});
				//console.log(JSON.parse(aData.procedimentos));
		...
		$("#buscaBanco").keyup(function () {
			if(this.value.length > 2){
				...
				timer = setTimeout(function() { // assign timer a new timeout
					x = table.fnReloadAjax('server/buscaOrcamento.php?busca='+$("#buscaBanco").val());
				...
				clearTimeout(timer); // Clear the timer so we don't end up with dupes.
				timer = setTimeout(function() { // assign timer a new timeout
					x = table.fnReloadAjax('server/buscaOrcamento.php');
					// run ajax request and store in x variable (so we can cancel)
				}, 500); // 2000ms delay, tweak for faster/slower
			}
		});
		...
		$('#datatable_col_reorder tbody').on( 'dblclick', 'tr', function () {
			loading("show");
			$.post("ajax/orcamento.php",{id:$(this).children('td').eq(0).text()}).done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '95%',
