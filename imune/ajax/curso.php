<?php
	...
	$nome 		= "Cursos";
	$colunas	= "id,nome,periodos,disciplinas";
	$tabela		= "curso";
								...
								while($row = mysqli_fetch_array($valida)){
									...
									<tr id="<?php echo 'tt'.$row[0];?>" style="cursor:pointer">
										<?php
											$explode = explode(",",$colunas);
											for($i=0;$i<count($explode);$i++){
												if($explode[$i] == 'disciplinas'){
													$disciplinas = explode(",",$row['disciplinas']);
													$nn = "<label class='label label-primary'>";
													foreach($disciplinas as $dis){
														$sel = "select nome from disciplina where id = '$dis'";
														$val = mysqli_query($con,$sel);
														if($r = mysqli_fetch_array($val)){
															$nn .= $r['nome']."</label> - <label class='label label-primary'>";

														};
													}
													$nn = substr($nn,0,strlen($nn)-38);
													$row[$i] = $nn;
												}
												?>
												<td>
