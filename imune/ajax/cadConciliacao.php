...
<div id="lancamentoConta" class="col-xs-12">
			...
			<div class="col-xs-12">
				<div class="col-xs-12">
					<label class="radio">
						...
						<span title='CONTAS À PAGAR' class='label label-danger text-center'>&nbsp;CONTAS À PAGAR&nbsp;&nbsp;</span>
					</label>
					<label class="radio">
						<input class="radiobox style-0" type="radio" name="tp_conta" value="2">
						<span title='CONTAS À RECEBER' class='label label-success text-center'>&nbsp;CONTAS À RECEBER&nbsp;&nbsp;</span>
					</label>
				</div>
			</div>
		</div>
	...
	<div class="row">
		...
		<div class="col-sm-3 col-xs-6">
			Nat. Financeira<select class="form-control" id="natureza">
				<option>ESCOLHA UM TIPO DE CONTA</option>
			</select>
		</div>
		<div class="col-sm-3 col-xs-6">
			Centro de Custo<select class="form-control" id="centrocusto">
				<option val>
					ESCOLHA UMA NATUREZA FINANCEIRA
				</option>
			</select>
		</div>
	</div>
	<div class="row">
		...
		<div class="col-sm-5 col-xs-9">
			Forma Pag.<select class="form-control" id="pagamento"></select>
		</div>
	...
	<div class="row hidden" id="baixando">
		<div class="col-sm-3 col-xs-12">
			Banco. Bx.<select class="form-control" id="bx_banco"></select>
		</div>
		<div class="col-sm-3 col-xs-9">
			Forma Pag. Bx.<select class="form-control" id="bx_pagamento"></select>
		</div>
		<div class="col-sm-2 col-xs-3" id="qtdqtd">
			Parc. Bx.<input class="form-control" id="bx_qtd" value="1">
		</div>
		<div class="col-sm-2 col-xs-6">
...
<div class="col-xs-12 center">
	<div class="row">
		<div class="col-xs-12">
			...
			<a href="javascript:void(0);" id="excluir" class="btn btn-sm btn-danger hidden"> <span class="btn-label"><i class="glyphicon glyphicon-repeat"></i></span>EXCLUIR</a>
...
<script>
	...
	$('input[name="tp_conta"]').change(function(){
		$.post("server/recupera.php",{tabela:'natureza  where tipo like "%'+this.value+'%" order by nome asc'}).done(function(data){
			...
			for(o in obj){
				if(o == 0){
					$("#lancamentoConta").find("#natureza").empty().append('<option selected value="'+obj[o].id+'">'+obj[o].nome+'</option>');
				}else{
					$("#lancamentoConta").find("#natureza").append('<option value="'+obj[o].id+'">'+obj[o].nome+'</option>');
				}
			};
			...
			$.post("server/recupera.php",{tabela:'centrocusto where '+query }).done(function(data){
				...
				for(o in obj){
					if(o == 0){
						$("#lancamentoConta").find("#centrocusto").empty().append('<option naturezas="'+obj[o].naturezas+'" value="'+obj[o].id+'">'+obj[o].nome+'</option>');
					}else{
						$("#lancamentoConta").find("#centrocusto").append('<option naturezas="'+obj[o].naturezas+'" value="'+obj[o].id+'">'+obj[o].nome+'</option>');
					}
				};
			}).fail(function(){
	...
	$("#lancamentoConta").find("#natureza").change(function(){
	...
	if(isset($_POST['id'])){
		...
		<script>
			...
			$("#salvar").click(function(){
				var tipo 		= $("#lancamentoConta").find("#tp_conta option:selected").val();
				var cli_id 		= $("#lancamentoConta").find("#cli_id").attr('retorno');
				var dt_cad		= $("#lancamentoConta").find("#dt_cad").val();
				var dt_fat		= $("#lancamentoConta").find("#dt_fat").val();
				var centrocusto	= $("#lancamentoConta").find("#centrocusto option:selected").val();
					...
					$.post('server/financeiro.php',{
						...
						if(data == 1){
							...
							$.post('ajax/gridContas.php',{type:tipo}).done(function(data){
								...
								alerta("Erro ao cadastrar conta!","Função não encontrada! Verifique sua conexão e tente novamente!","danger","warning");
							});
						}else{
							loading('hide');
							alerta("Erro ao cadastrar conta!","Favor verifique todos os campos e tente novamente, se o problema continuar, contate o suporte técnico!","danger","warning");
...
if(isset($_POST['tipo'])){
	...
	<script>
		...
		$("#baixar").click(function(){
			...
			$("#lancamentoConta").find("#bx_pagamento,#bx_banco,#dt_baixa,#pr_baixa").removeAttrs('disabled');
			$("#pr_baixa").val($("#pr_liquido").val());


			$("#lancamentoConta").find("#bx_pagamento").change(function(){
				var condicao       = $("#lancamentoConta").find("#bx_pagamento option:selected").attr('condicao');
				if(condicao == 2){
					$("#lancamentoConta").find("#bx_qtd").removeAttr("disabled");
				}else{
					$("#lancamentoConta").find("#bx_qtd").attr("disabled",true);
				};
			});
			...
			$("#salvar2").click(function(){
				...
				var qtd        = $("#lancamentoConta").find("#bx_qtd").val();
		...
		$("#replicar").click(function(){
			$("#editar,#replicar,#baixar").addClass("hidden");
