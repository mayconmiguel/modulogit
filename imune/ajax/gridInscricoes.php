...
$tabelinha  = array("id","nome","cpf","telefone","celular","email","curso1","curso2","obs","origem");
?>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/inscricoes.php').done(function(data){
		...
		var table = $('#datatable_col_reorder').dataTable({
			...
			"order":[[0,'asc']],
			"ajax": "server/buscaInscricoes.php",
			"columns": cliente,
			oSearch: {"bRegex": true},
			"iDisplayLength": 10,
			"autoWidth" : true,
		...
		$('#datatable_col_reorder tbody').on( 'dblclick', 'tr', function () {
			loading("show");
			$.post("ajax/inscricoes.php",{id:$(this).find('td:first').text()}).done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '95%',
