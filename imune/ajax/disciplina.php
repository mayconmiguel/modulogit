<?php
	...
	$nome 		= "Disciplina";
	$colunas	= "id,nome,carga,valor";
	$tabela		= "disciplina";

	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tabela." where grp_emp_id = '".$_SESSION['imunevacinas']['usuarioEmpresa']."'";
		}else{
			$select = "select ".$colunas." from ".$tabela." where id = '$busca' or nome like '%$busca%' and grp_emp_id = '".$_SESSION['imunevacinas']['usuarioEmpresa']."'";
		}
	}else{
		$select = "select ".$colunas." from ".$tabela." where grp_emp_id = '".$_SESSION['imunevacinas']['usuarioEmpresa']."'";
	};
								...
								<tr>
									...
									for($i=0;$i<count($explode);$i++){
										if($explode[$i] == "nome"){
											...
											<?php
										}elseif($explode[$i] == "carga"){
											$explode[$i] = "CARGA HORÁRIA";
											?>
											<th data-hide="phone">
												<?php echo mb_strtoupper($explode[$i]);?>
											</th>
											<?php
										}else{
											?>
