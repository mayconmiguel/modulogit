<?php
	...
	if($_GET['type'] == 1){
		$nome 		= $_SESSION['config']['funcionario'];
		$colunas	= "id,nome,cpf,telefone,celular,email";
		$tab		= "pessoa where produtor = 1 ";
	}elseif($_GET['type'] == 2){
		$nome 		= "Fornecedor";
		$colunas	= "id,nome,cpf,telefone,celular,email";
		$tab		= "pessoa where fornecedor = 1 ";
	}
	$type       = $_GET['type'];
	$tabela		= "produtor";
	...
	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tab." and grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id";
		}else{
			$select = "select ".$colunas." from ".$tab." and (id = '$busca' or nome like '%$busca%') and grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']."";
		}
	}else{
		$select = "select ".$colunas." from ".$tab." and grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by id";
	}
...
<section id="widget-grid-<?php echo $tabela;?>" class="well">
	<div class="jarviswidget jarviswidget-color-green" id="wid-id-2" data-widget-fullscreenbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-colorbutton="false">
		<header>
			<h2>Cadastro <span>> <?php echo $nome;?></span></h2>
		</header>
							...
							<thead>
								<tr>
									...
									for($i=0;$i<count($explode);$i++){
										if($explode[$i] == "nome" || $explode[$i] == "razao"){
											?>
								...
								while($row = mysqli_fetch_array($valida)){
									...
									<script>
										$("#tt<?php echo $row[0];?>").dblclick(function(){
											$.post("ajax/cad<?php echo $tabela;?>.php",{id:<?php echo $row[0];?>,type:<?php echo $type;?>}).done(function(data){
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		$("#todos").click(function(){
			loading('show');
			$.ajax({
				url: 'ajax/<?php echo $tabela;?>.php?type=<?php echo $type;?>&busca=PR1SC1L4',
				type: 'GET',
				data:{busca:'PR1SC1L4'},
				cache: false,
		...
		$("#pesquisa").click(function(){
			if($("#busca").val().length < 3){
				...
				$.ajax({
					url: 'ajax/<?php echo $tabela;?>.php?type=<?php echo $type;?>&busca='+$("#busca").val(),
					type: 'GET',
					cache: false,
					success: function(data) {
						$("#content").html(data);
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/cad<?php echo $tabela;?>.php',{type:<?php echo $type;?>}).done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '95%',
