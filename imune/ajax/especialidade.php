<?php
	...
	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tabela." where grp_emp_id = '$empresa' order by id";
		}else{
			$select = "select ".$colunas." from ".$tabela." where grp_emp_id = '$empresa' and id = '$busca' or nome like '%$busca%' order by id";
		}
	}else{
		$select = "select ".$colunas." from ".$tabela." where grp_emp_id = '$empresa' order by id asc limit 50";
	}
								...
								while($row = mysqli_fetch_array($valida)){
									...
									<script>
										$("#tt<?php echo $row[0];?>").dblclick(function(){
											$.post("ajax/cadEspecialidade.php",{id:<?php echo $row[0];?>}).done(function(data){
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/cadEspecialidade.php').done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '90%',
