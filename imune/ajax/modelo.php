<?php
	...
	$nome 		= "Modelo";
	$colunas	= "modelo.id,modelo.nome as modelo,marca.nome as marca";
	$tabela		= "modelo,marca";

	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tabela." where marca.id = modelo.mar_id and modelo.grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by modelo.nome asc";
		}else{
			$select = "select ".$colunas." from ".$tabela." where marca.id = modelo.mar_id and (modelo.id = '$busca' or modelo.nome like '%$busca%') and modelo.grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by modelo.nome asc";
		}
	}else{
		$select = "select ".$colunas." from ".$tabela." where marca.id = modelo.mar_id and modelo.grp_emp_id = ".$_SESSION['imunevacinas']['usuarioEmpresa']." order by modelo.nome asc";
	}
					...
					<div class="widget-body no-padding">
						...
						<table id="datatable_col_reorder" class="table table-striped table-bordered table-hover" width="100%">
							<thead>
								<tr>
									<?php
									$cols = 'id,modelo,marca';
									$explode = explode(",",$cols);
								...
								while($row = mysqli_fetch_array($valida)){
									...
									<script>
										$("#tt<?php echo $row[0];?>").dblclick(function(){
											$.post("ajax/cadmodelo.php",{id:<?php echo $row[0];?>}).done(function(data){
	...
	var pagefunction = function() {
		...
		$("#pesquisa").click(function(){
			if($("#busca").val().length < 2){
				alerta("AVISO!","Favor digitar pelo menos 2 caracteres para realizar a busca!","warning","warning");
				focar("#busca");
			}else{
				$.ajax({
					url: 'ajax/modelo.php',
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/cadmodelo.php').done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '95%',
