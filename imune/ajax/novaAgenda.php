<?php
	...
	$nome 		= "Abertura de Agenda";
	$colunas	= "agenda.id,concat(agenda.prefixo,' ',pessoa.nome),agenda.slotDuration,agenda.almoco,agenda.minTime,agenda.maxTime";
	$tabela		= "agenda,pessoa";
	$empresa    = $_SESSION['imunevacinas']['usuarioEmpresa'];

	if(isset($_GET['busca'])){
		$busca = $_GET['busca'];
		if($busca == "PR1SC1L4"){
			$select = "select ".$colunas." from ".$tabela." where pessoa.id = agenda.pes_id and agenda.grp_emp_id = '$empresa' order by id";
		}else{
			$select = "select ".$colunas." from ".$tabela." where pessoa.id = agenda.pes_id and agenda.grp_emp_id = '$empresa' and id = '$busca' or nome like '%$busca%' order by id";
		}
	}else{
		$select = "select ".$colunas." from ".$tabela." where pessoa.id = agenda.pes_id and agenda.grp_emp_id = '$empresa' order by id asc limit 50";
	}
	$colunas = 'id,nome,intervalo,ALMOÇO,inicio,fim';
...
<!-- widget grid -->
<section id="widget-grid-agenda" class="well">
		...
		<div class="col-lg-4 col-md-8 col-sm-10 col-xs-10">
			<form action="" class="smart-form" style="background:rgba(255,255,255,0)">
				<fieldset style="background:rgba(255,255,255,0)">
					<section>
						<label class="input">
							<input type="text" id="busca" class="input-lg" placeholder="Digite o nome.">
								...
								while($row = mysqli_fetch_array($valida)){
									...
									<script>
										$("#tt<?php echo $row[0];?>").dblclick(function(){
											loading('show');
											$.post("ajax/cadAgenda.php",{id:<?php echo $row[0];?>}).done(function(data){
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/cadAgenda.php').done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '90%',
