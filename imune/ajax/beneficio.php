<?php
	...
	$tabelinha  = array("id","nome","especialidade","procedimento","tipo","valor");
?>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		// Dialog click
		$('#novo').click(function() {
			$.post('ajax/cadBeneficio.php').done(function(data){
		...
		var table = $('#datatable_col_reorder').dataTable({
			...
			"order":[[ 5, 'asc' ],[0,'asc']],
			"ajax": "server/buscaBeneficio.php",
			"columns": cliente,
			oSearch: {"bRegex": true},
			"iDisplayLength": 10,
			"autoWidth" : true,
		...
		$("#buscaBanco").keyup(function () {
			if(this.value.length > 2){
				...
				timer = setTimeout(function() { // assign timer a new timeout
					x = table.fnReloadAjax('server/buscaBeneficio.php?busca='+$("#buscaBanco").val());
				...
				clearTimeout(timer); // Clear the timer so we don't end up with dupes.
				timer = setTimeout(function() { // assign timer a new timeout
					x = table.fnReloadAjax('server/buscaBeneficio.php');
					// run ajax request and store in x variable (so we can cancel)
				}, 500); // 2000ms delay, tweak for faster/slower
			}
		});
		...
		$('#datatable_col_reorder tbody').on( 'dblclick', 'tr', function () {
			loading("show");
			$.post("ajax/cadBeneficio.php",{id:$(this).find('#id').attr('valor')}).done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '95%',
