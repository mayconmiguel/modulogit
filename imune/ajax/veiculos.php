<?php
	...
	$tabelinha  = array("ID","imagem","marca","modelo","cor","valor");
?>
...
<script type="text/javascript">
	...
	var pagefunction = function() {
		...
		$("#buscaBanco").keyup(function () {
			if(this.value.length > 2){
				...
				timer = setTimeout(function() { // assign timer a new timeout
					x = table.fnReloadAjax('server/buscaVeiculo.php?busca='+$("#buscaBanco").val());
				...
				clearTimeout(timer); // Clear the timer so we don't end up with dupes.
				timer = setTimeout(function() { // assign timer a new timeout
					x = table.fnReloadAjax('server/buscaVeiculo.php');
					// run ajax request and store in x variable (so we can cancel)
				}, 500); // 2000ms delay, tweak for faster/slower
			}
		});
		...
		$('#datatable_col_reorder tbody').on( 'dblclick', 'tr', function () {
			loading("show");
			$.post("ajax/cadveiculos.php",{id:$(this).find('#id').attr('valor')}).done(function(data){
				$("#cadastro").empty().html(data);
				$("#cadastro").dialog({
					autoOpen : true,
					width : '95%',
