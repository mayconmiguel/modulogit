...
// validando se é inclusão, exclusão ou edição de dados.
    ...
    if($funcao == 1)
    {
        $select = "select * from cor where grp_emp_id = $grupoEmpresa and  nome = '$nome'";
        ...
        {
            $insert = "insert ignore into cor(grp_emp_id,nome)values('$grupoEmpresa','$nome')";
    ...
    elseif($funcao == 2)
    {
        $update = "update ignore cor set nome = '$nome' where id = '$id'";
    ...
    elseif($funcao == 3)
    {
        $delete = "delete from cor where id = '$id'";
