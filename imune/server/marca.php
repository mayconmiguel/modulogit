...
// validando se é inclusão, exclusão ou edição de dados.
    ...
    if($funcao == 1)
    {
        $select = "select * from marca where grp_emp_id = $grupoEmpresa and  nome = '$nome'";
        ...
        {
            $insert = "insert ignore into marca(grp_emp_id,nome)values('$grupoEmpresa','$nome')";
    ...
    elseif($funcao == 2)
    {
        $update = "update ignore marca set nome = '$nome' where id = '$id'";
