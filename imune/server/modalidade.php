...
// validando se é inclusão, exclusão ou edição de dados.
    ...
    if($funcao == 1)
    {
        $select = "select * from modalidade where grp_emp_id = $grupoEmpresa and  nome = '$nome'";
        ...
        {
            $insert = "insert ignore into modalidade(grp_emp_id,nome)values('$grupoEmpresa','$nome')";
    ...
    elseif($funcao == 2)
    {
        $update = "update ignore modalidade set nome = '$nome' where id = '$id'";
    ...
    elseif($funcao == 3)
    {
        $delete = "delete from modalidade where id = '$id'";
